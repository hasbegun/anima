...
setup:
	...
	@until docker inspect anima-thunderid-1 --format '{{.State.Health.Status}}' 2>/dev/null | grep -q healthy; do \
...
status:
	docker compose ps
	@echo ""
	@docker inspect anima-thunderid-1 --format '{{.State.Health.Status}}' 2>/dev/null \
		| grep -q healthy \
