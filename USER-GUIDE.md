...
Anima is a centralized identity and authorization gateway for internal servers, built on **ThunderID**. Provides shared authentication for humans and AI agents with per-tenant RBAC, OAuth2/OIDC token issuance, and offline JWT verification.
...
Anima treats AI agents as **first-class citizens** in the same identity system as humans. Every agent gets:
...
```text
NAME                     IMAGE                                  STATUS
anima-thunderid-1     ghcr.io/thunder-id/thunderid:1.0.1     Up 2 minutes (healthy)
...
docker inspect anima-thunderid-1 --format '{{.Config.Image}}'
